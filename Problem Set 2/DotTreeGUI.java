...
public class DotTreeGUI extends DrawingGUI {
	...
	/**
	 * Check the size and allPoints methods to ensure they are working properly
	 */
	private void test2() {
		found = null;
		tree = new PointQuadtree<Dot>(new Dot(100,200), 0,0,800,600);
		tree.insert(new Dot(150,450));
		tree.insert(new Dot(250,550));
		tree.insert(new Dot(350,175));
		tree.insert(new Dot(500,125));
		tree.insert(new Dot(475,250));
		tree.insert(new Dot(490,215));
		tree.insert(new Dot(700,550));
		tree.insert(new Dot(310,410));
		if(tree.size() != 9) {
			System.err.println("Tree's size is wrong! Expected a size of 9, but got a size of " + tree.size());
		}
		else if(tree.allPoints().size() != 9) {
			System.err.println("Tree's allPoints method doesn't work! Expected it to return 9 points, but recieved " + tree.allPoints().size());
		}
		else {
			System.out.println("Test 2 passed! Size and allPoints methods work properly.");
